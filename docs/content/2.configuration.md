...
```typescript [components/CookieBanner.vue]
<template>
  <Transition name="fade">
    <div v-if="isConsentPending()" class="cookie-banner">
...
const { isConsentPending, optIn, optOut, getConsentStatus } = usePostHogConsent();
...
const status = getConsentStatus(); // 'pending' | 'granted' | 'denied'
</script>
```

### Available Methods

The `usePostHogConsent` composable provides the following methods:

- `getConsentStatus()` - Returns the current consent status: `'pending' | 'granted' | 'denied'`
- `isConsentPending()` - Returns `true` if consent is pending (banner should be shown)
- `hasConsent()` - Returns `true` if consent has been granted
- `isConsentDenied()` - Returns `true` if consent has been denied
- `optIn()` - Grant consent and enable tracking
- `optOut()` - Deny consent and disable tracking (cookieless tracking continues)

### Limitations

When using cookieless mode, be aware of these limitations:

- **Session replay and surveys** are disabled if the user has not given cookie consent
...
- **Higher unique user counts** may appear as users are tracked differently each day

For more details, see PostHog's [cookieless tracking documentation](https://posthog.com/tutorials/cookieless-tracking).
